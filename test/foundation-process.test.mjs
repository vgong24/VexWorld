...
test('current repository control sources are internally valid', async () => {
  ...
  assert.deepEqual(
    validateVersionBinding({ repositoryManifest, currentVersion, projectState }),
    []
  );
});
...
test('technology candidates cannot become adopted from a name or preference alone', async () => {
  ...
  const selected = broken.candidates.find(
...
test('accepted asset intake requires exact downloaded artifact, hash, license and local path', async () => {
  ...
  const incomplete = Object.fromEntries(
    policy.requiredIntakeFields.map((field) => [field, null])
  );
  incomplete.assetCandidateRef = 'asset-candidate.synthetic.fixture';
  incomplete.displayName = 'Synthetic fixture';
  incomplete.assetClass = 'TEST_ONLY';
  incomplete.officialSourceUrl = 'https://example.invalid/fixture';
  incomplete.sourcePublisherRef = 'publisher.synthetic';
  incomplete.artifactVersionOrRelease = '1.0.0';
  incomplete.licenseSpdxOrExactIdentifier = 'CC0-1.0';
  incomplete.licenseSourceUrl = 'https://example.invalid/license';
  incomplete.permittedUseSummary = 'Synthetic test only';
  incomplete.attributionRequirement = 'NONE';
  incomplete.modificationRecordRefs = [];
  incomplete.intendedVexWorldUse = 'Contract test';
  incomplete.disposition = 'ACCEPTED_REFERENCE_STRUCTURE_ONLY';
...
test('glob matching treats engine, version and root paths as distinct explicit routes', () => {
...
test('current-version drift is rejected instead of silently selecting a nearby version', async () => {
  ...
  const errors = validateVersionBinding({
    repositoryManifest,
